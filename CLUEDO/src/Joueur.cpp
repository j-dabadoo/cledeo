...
void Joueur::deplacement(BITMAP* buffer, std::vector<std::vector <char>> &matrice)
{
    ...
    if(nb_depl>0 and (key[KEY_UP] or key[KEY_DOWN] or key[KEY_RIGHT] or key[KEY_LEFT]))
    {
        ...
        while(key[KEY_UP] or key[KEY_DOWN] or key[KEY_RIGHT] or key[KEY_LEFT]){}
