...
void Game::jeu()
{
    ...
    while(!key[KEY_ESC] and quitter==false)
    {
        ...
        if(joueurs[tour].getLoca() == 'C' and joueurs[tour].getFin() == false)
            joueurs[tour].setFin(casechance(plateau, joueurs, tour, nb_joueurs, buffer, matrice, fond, broadway, des));
...
bool Game::casechance(BITMAP* plateau, std::vector<Joueur>& joueurs, int& tour, int nb_joueurs, BITMAP* buffer,
                      std::vector<std::vector<char>> matrice, BITMAP* fond, FONT* broadway, BITMAP* des)
{
    int chance = rand()%4;
